...
export const fetchHelpCenterContent = async (inputString: string) => {
    ...
    let articlesListEndpoint = `https://${zendeskSubdomain}.zendesk.com/api/v2/help_center/articles/`;
    if(pageToken){
...
const fetchSection = async (sectionId: any) => await fetch(`https://${zendeskSubdomain}.zendesk.com/api/v2/help_center/sections/${sectionId}`).then(response => response.json());
const fetchCategory = async (categoryId: any) => await fetch(`https://${zendeskSubdomain}.zendesk.com/api/v2/help_center/categories/${categoryId}`).then(response => response.json());